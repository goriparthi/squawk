...
/// The window is rectangular; only the dial is painted. macOS routes mouse
...
///
/// This is the face style's renderer. A companion with a body is modelled
/// rather than drawn, and then this view paints nothing at all.
final class CircleBackgroundView: NSView {
    /// True when the modelled companion is on screen, in which case this view
    /// paints nothing and takes no clicks of its own.
    var isModelled = false {
        didSet {
            guard isModelled != oldValue else { return }
            needsDisplay = true
        }
    }
    ...
    override func draw(_ dirtyRect: NSRect) {
        // Nothing at all when the modelled companion is on screen: it draws its
        // own head, and the flat one behind it was a dark disc the size of the
        // whole window.
        guard !isModelled else { return }
    ...
    override func hitTest(_ point: NSPoint) -> NSView? {
        ...
        // Three things decide their own clicks: the bubble, the card it holds,
        // and the modelled pet, which knows its own silhouette far better than
        // any shape this view could test. Everything else falls through to the
        // drawn head below.
        for overlay in subviews.reversed()
        where overlay is BubbleView || overlay is DetailView || overlay is CompanionView {
            guard !overlay.isHidden, let hit = overlay.hitTest(local) else { continue }
            return hit
        }
        guard !isModelled else { return nil }
        let head = headFrame
